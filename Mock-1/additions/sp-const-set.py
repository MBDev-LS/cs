...
def setConsts():
	global SOIL, SEED, PLANT, ROCKS, FIELDLENGTH, FIELDWIDTH
	useDefaultRes = input('Would you like to use the default settings? (Y/N): ')

	while useDefaultRes not in ['Y', 'N']:
		useDefaultRes = input('Would you like to use the default settings? (Y/N): ')

	if useDefaultRes == 'Y':
		return
	
	newSoil = input('Enter soil character (\'skip\' to leave default): ')
	SOIL = '.' if newSoil == 'skip' else newSoil

	newSeed = input('Enter seed character (\'skip\' to leave default): ')
	SEED = 'S' if newSeed == 'skip' else newSeed

	newPlant = input('Enter plant character (\'skip\' to leave default): ')
	PLANT = 'P' if newPlant == 'skip' else newPlant

	newRock = input('Enter rock character (\'skip\' to leave default): ')
	ROCKS = 'X' if newRock == 'skip' else newRock

	newFieldLength = input('Enter the new field length (\'skip\' to leave default): ')
	
	FIELDLENGTH = 20 if newFieldLength == 'skip' or not newFieldLength.isdigit() else newFieldLength

	newFieldWidth = input('Enter the new field length (\'skip\' to leave default): ')
	FIELDWIDTH = 35 if newFieldWidth == 'skip' or not newFieldWidth.isdigit() else newFieldWidth



def CreateNewField():
	print(FIELDWIDTH, FIELDLENGTH)
