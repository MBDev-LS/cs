...
def addRocks(field: list, numberOfRocks: int) -> list:
	available = []
	for y in range(FIELDLENGTH):
		for x in range(FIELDWIDTH):
			if y == FIELDLENGTH // 2 and x == FIELDWIDTH // 2:
				continue
			available.append((x, y))

	for i in range(numberOfRocks):
		nextPlacement = choice(available)
		available.remove(nextPlacement)
		
		field[nextPlacement[1]][nextPlacement[0]] = ROCKS
...
def CreateNewField(): 
	...
	rocksNumberInput = input(f'Enter the number of rocks you would like to generate (Must be less than or equal to {FIELDLENGTH*FIELDWIDTH-1}): ')
	while True:
		if rocksNumberInput.isdigit():
			if int(rocksNumberInput) <= FIELDLENGTH*FIELDWIDTH-1:
				break
		rocksNumberInput = input(f'Enter the number of rocks you would like to generate (Must be less than or equal to {FIELDLENGTH*FIELDWIDTH-1}): ')
	
	Field = addRocks(Field, int(rocksNumberInput))
