	...
	for Row in range(FIELDLENGTH):
		for Column in range(FIELDWIDTH):
			if Field[Row][Column] == SEED:
				NumberOfSeeds += 1
	if NumberOfSeeds == 1:
		print('There is 1 seed growing')
	else:  
		print('There are', NumberOfSeeds, 'seeds growing')
...
def SimulateOneYear(Field, Year):
	...
	for season in seasons:
		...
		Response = input('Press Enter to run simulation for another Season, Input X to stop: ')
