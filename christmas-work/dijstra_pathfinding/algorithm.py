...
while finished is not True:
    ...
    print('Working Stack: [', ', '.join([str(node) for node in working_stack]) + ' ]')
    print('Finished List: [', ', '.join([str(node) for node in finished_list]) + ' ]')
...
while final_path[-1].start is not True:
    final_path.append(finished_list.pop(finished_list.index(final_path[-1].previous_node)))
