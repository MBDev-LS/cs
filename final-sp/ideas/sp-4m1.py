...
# Idea: Reject non-int inputs when getting square reference
...
class Dastan:
	...
	def __GetSquareReference(self, Description):
		SelectedSquare = input("x the square " + Description + " (row number followed by column number): ")

		while not SelectedSquare.isdigit():
			print('Please enter a two valid (consecutive) integers.')
			SelectedSquare = input("x the square " + Description + " (row number followed by column number): ")
