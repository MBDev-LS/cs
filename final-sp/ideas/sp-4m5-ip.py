...
# Idea: Allow players to set their names during setup
# Author: Damian
...
class Dastan:
	def __init__(self, R, C, NoOfPieces):
		...
		self._PrevPlr = None
		self._Players.append(Player(self._GetName(1), 1))
		self._Players.append(Player(self._GetName(2), -1))
	...
	def _GetName(self, playerId: str):
		inputtedName = input(f'Please enter a name for player {playerId}: ')
		while len(inputtedName.strip()) == 0:
			print("dont put nothing fool")
			inputtedName = input(f'Please enter a name for player {playerId}: ')
		return f'{inputtedName} (Player {playerId})'
