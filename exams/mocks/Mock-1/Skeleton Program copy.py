...
def SimulateAutumn(Field):
	...
	for Row in range(FIELDLENGTH):
		for Column in range(FIELDWIDTH):
			if Field2[Row][Column] == PLANT:
				Field2 = SeedLands(Field2, Row - 1, Column - 1)
				Field2 = SeedLands(Field2, Row - 1, Column)
				Field2 = SeedLands(Field2, Row - 1, Column + 1)
				Field2 = SeedLands(Field2, Row, Column - 1)
				Field2 = SeedLands(Field2, Row, Column + 1)
				Field2 = SeedLands(Field2, Row + 1, Column - 1)
				Field2 = SeedLands(Field2, Row + 1, Column)
				Field2 = SeedLands(Field2, Row + 1, Column + 1)
	
	Display(Field2, 'TESTING', 'TESTING')



	print(f'Wind Direction: {windDirections[yMod][xMod]}')

	for Row in range(FIELDLENGTH):
		for Column in range(FIELDWIDTH):
			if Field[Row][Column] == PLANT:
				Field = SeedLands(Field, Row - 1 + xMod, Column - 1 + yMod)
				print(f'OLD X: {Row - 1}, NEW: {Row - 1 + xMod}, MOD: {xMod}')
				print(f'OLD Y: {Column - 1}, NEW: {Column - 1 + yMod}, MOD: {yMod}')
				Field = SeedLands(Field, Row - 1 + xMod, Column + yMod)
				print(f'OLD X: {Row - 1}, NEW: {Row - 1 + xMod}, MOD: {xMod}')
				print(f'OLD Y: {Column - 1}, NEW: {Column - 1 + yMod}, MOD: {yMod}')
				Field = SeedLands(Field, Row - 1 + xMod, Column + 1 + yMod)
				print(f'OLD X: {Row - 1}, NEW: {Row - 1 + xMod}, MOD: {xMod}')
				print(f'OLD Y: {Column - 1}, NEW: {Column - 1 + yMod}, MOD: {yMod}')
				Field = SeedLands(Field, Row + xMod, Column - 1 + yMod)
				print(f'OLD X: {Row}, NEW: {Row + xMod}, MOD: {xMod}')
				print(f'OLD Y: {Column}, NEW: {Column + yMod}, MOD: {yMod}')
				Field = SeedLands(Field, Row + xMod, Column + 1 + yMod)
				print(f'OLD X: {Row}, NEW: {Row + xMod}, MOD: {xMod}')
				print(f'OLD Y: {Column}, NEW: {Column + yMod}, MOD: {yMod}')
				Field = SeedLands(Field, Row + 1 + xMod, Column - 1 + yMod)
