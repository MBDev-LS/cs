...
def Display(Field, Season, Year):
	...
	columnWidth = len(str(FIELDWIDTH))+1

	topNumberRow = ''.join([f'{i:02d} ' for i in range(FIELDWIDTH)])

	print(topNumberRow)

	print('_'*(len(topNumberRow)+1))

	# print('_'*round(FIELDLENGTH*2*0.9))
	for Row in range(FIELDLENGTH):
		for Column in range(FIELDWIDTH):
			print(Field[Row][Column] + ' '*(columnWidth-1), end='')
