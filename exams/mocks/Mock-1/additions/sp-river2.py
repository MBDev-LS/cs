...
PLANT = 'P'
ROCKS = 'X'
RIVER = '~' # Changes to 'R' for easier viewing
...
def placeRiverUnit(Field: list, ix, iy) -> list:
	Field[iy][ix] = RIVER

def drawRiver(Field):
	if choice([True, False]) is True:
		startY = randint(0, FIELDLENGTH - 1)
		...
		for x in range(FIELDWIDTH):
			if Field[y][x] != SOIL:
				y += choice([-1, 1])
			if y > 0 and y < FIELDLENGTH - 1:
				placeRiverUnit(Field, x, y)
				y += randint(-1, 1)
			elif y == 0:
				y += 1
				placeRiverUnit(Field, x, y)
			elif y == FIELDLENGTH - 1:
				y -= 1
				placeRiverUnit(Field, x, y)
	else:
		startX = randint(0, FIELDWIDTH - 1)
...
def CreateNewField(): 
	...
	includeRiver = input('Include river in field (Y/N): ')
	while includeRiver not in ['Y', 'N']:
		includeRiver = input('Include river in field (y/n): ')

	if includeRiver == 'Y':
		Field = drawRiver(Field)
