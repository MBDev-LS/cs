...
def FieldToFile(Field):
	fileText = ''
	for Row in range(FIELDLENGTH):
		for Column in range(FIELDWIDTH):
			fileText += Field[Row][Column]
		fileText += '|{0:>3}'.format(Row) + '\n'
	fileText += '\n'
...
def saveField(Field):
	fileName = input('Enter the file name, this will saved as <filename>.txt: ')
	with open(f'{fileName}.txt', 'w+') as f:
		f.write(FieldToFile(Field))
...
def Simulation():
	...
	fieldToBeSaved = input('Would you like to save this field to a .txt file? (y/n): ').lower()
	while fieldToBeSaved not in ['y', 'n']:
		fieldToBeSaved = input('Would you like to save this field to a .txt file? (y/n): ').lower()
