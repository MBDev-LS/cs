...
def plotRiverPoint(field, x, y):
	field[y][x] = RIVER
	print(len(field), field[y][x])
	print(field[0][34])

	return field

def plotRiverLow(x0, y0, x1, y1, field):
	dx = x1 - x0
	dy = y1 - y0
	yi = 1
	...
	for x in range(x0, x1):
		field = plotRiverPoint(field, x, y)
		if D > 0:
			y += yi
			D += (2 * (dy - dx))
		else:
...
def plotRiverHigh(x0, y0, x1, y1, field):
	dx = x1 - x0
	dy = y1 - y0
	xi = 1
...
def pickPlot(x0, y0, x1, y1, field):
	if abs(y1 - y0) < abs(x1 - x0):
		if x0 > x1:
			return plotRiverLow(x1, y1, x0, y0, field)
		else:
			return plotRiverLow(x0, y0, x1, y1, field)
	else:
		if y0 > y1:
			return plotRiverHigh(x1, y1, x0, y0, field)
		else:
			return plotRiverHigh(x0, y0, x1, y1, field)
...
def CreateNewField(): 
	...
	while not valid:
		...
		if choice(['horizontal', 'vertical']) == 'horizontal':
			leftPoint = randint(0, FIELDLENGTH - 1)
			rightPoint = randint(0, FIELDLENGTH - 1)

			fieldWithRiver = pickPlot(0, leftPoint, FIELDWIDTH - 1, rightPoint, Field)
		else:
			topPoint = randint(0, FIELDLENGTH - 1)
			bottomPoint = randint(0, FIELDLENGTH - 1)

			fieldWithRiver = pickPlot(topPoint, 0, bottomPoint, FIELDLENGTH - 1, Field)

		if fieldWithRiver[Row][Column] == SEED:
			valid = True

	return fieldWithRiver
